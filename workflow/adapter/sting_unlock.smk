...
rule update_bilan:
    ...
    shell: "module load python ; /mnt/beegfs/software/python/3.9.1/bin/python3.9 workflow/adapter/sting_unlock/update_bilan.py --bilan {input.bilan} --dataset {input.dataset} --rebuild {output.rebuild} --batch {params.batch} --out {log.out} "
...
rule build_variant_call_table:
    ...
    resources:
        ...
        mem_mb= 1024,
    # conda: "/mnt/beegfs/userdata/j_wang/.conda/envs/python3"
    shell: "module load python ; /mnt/beegfs/software/python/3.9.1/bin/python3.9 workflow/adapter/sting_unlock/build_variant_call_table.py --bilan {input.bilan} --dataset {input.dataset} --batch {params.batch}  --variant {output.variant} --out {log.out} "
...
rule build_rna_samplesheet_table:
    ...
    resources:
        ...
        mem_mb= 1024,
    # conda: "/mnt/beegfs/userdata/j_wang/.conda/envs/python3"
    shell: "module load python ; /mnt/beegfs/software/python/3.9.1/bin/python3.9 workflow/adapter/sting_unlock/build_rna_samplesheet_table.py --bilan {input.bilan} --batch {params.batch} --sheet {output.sheet} --out {log.out} "
...
rule build_dataset_by_bilan:
    ...
    resources:
        ...
        mem_mb= 1024,
    # conda: "/mnt/beegfs/userdata/j_wang/.conda/envs/python3"
    shell: "module load python ; /mnt/beegfs/software/python/3.9.1/bin/python3.9 workflow/adapter/sting_unlock/rebuild_dataset_by_bilan.py --bilan {input.bilan} --variant {input.variant} --sheet {input.sheet} --dataset {output.dataset} --batch {params.batch} --out {log.out} "
