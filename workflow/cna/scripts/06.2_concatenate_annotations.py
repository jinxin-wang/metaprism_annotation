...
def build_row_identifier_maf(df, col_gene="Hugo_Symbol", chrom="Chromosome", col_start="Start_Position", col_end="End_Position",
                             col_ref="Reference_Allele", col_alt="Tumor_Seq_Allele2"):
    if df.shape[0]>0:
        dfc = df.copy()
        cols = ["Tumor_Sample_Barcode", "Matched_Norm_Sample_Barcode", chrom, col_gene, col_start, col_end, col_ref, col_alt]
...
def main(args):
    ...
    print("-INFO: there are %d rows in OncoKB " % len(df_okb["Row_Identifier"]))
    print("-INFO: there are %d rows in CIViC  " % len(df_civ["Row_Identifier"]))
    ...
    # check
    print("-INFO: there are %d merged rows " % df_ann.shape[0])
